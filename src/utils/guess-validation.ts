...
export const checkGuessedWord = async (
  ...
  if (normalizedMessage.includes(' ')) {
    return [`"${message}" contains multiple words. Please guess a single word!`, WordStatus.NOT_GUESSED];
  }

  // Check if the guessed word is the plural of the current word
  if (normalizedMessage === pluralize(TargetWord)) {
    return [`${message} 🥳`, WordStatus.GUESSED]; // Plural guessed correctly
  }

  // Exact match check
  if (normalizedMessage === TargetWord) {
    return [`${message} ✅ You guessed the word!`, WordStatus.GUESSED];
  }

  // Check for synonyms using Datamuse API 
  const isSynonym = await checkIfSynonym(currentWord, normalizedMessage);
  if (isSynonym) {
    return [`${message} 🔄 Correct! It's a synonym of the word: ${TargetWord}`, WordStatus.GUESSED];
  }

  // Levenshtein Distance check (allowing some misspellings)
  if (
    normalizedMessage.length > MIN_LEVENSHTEIN_WORD_LENGTH &&
    calculateDamerauLevenshteinDistance(normalizedMessage, TargetWord) <= GUESSER_LEVENSHTEIN_THRESHOLD
  ) {
    return [
  ...
  return [`"${message}": wrong word. Try again!`, WordStatus.NOT_GUESSED];
};

// Helper function to check synonyms using Datamuse API
const checkIfSynonym = async (currentWord: string, guessedWord: string): Promise<boolean> => {
  try {
    const response = await axios.get(`https://api.datamuse.com/words?ml=${currentWord}`);
    const synonyms = response.data.map((entry: { word: string }) => entry.word.toLowerCase());
